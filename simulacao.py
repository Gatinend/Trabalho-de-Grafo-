...
qtd = int(input("Quantos caminhões ? "))
...
while ativos:
        ...
        for viz_nome, peso in no.adjacentes:
            ...
            if (
                ...
                if isinstance(viz, Arvore):
                    print(f"{viz_nome} já foi {viz._estado_final} ou ja esta queimando e não pode queimar de novo.")
    ...
    tempo += 1
    time.sleep(2)  # opcional: pausa de 2s entre passos
...
for n in queimados:
    print(f" - {n}")
    print(f"Parte salva (potência mínima): {no.potencia_queima.minimo}")
